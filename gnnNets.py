...
def get_gnnNets(input_dim, output_dim, model_config, concept_whitening, concept_acts=False):
    model_types = ['gcn', 'gat', 'gin']

    if model_config.gnn_name.lower() in model_types:
        ...
        return GCNNet(input_dim=input_dim,
                      ...
                      model_type=model_config.gnn_name.lower(),
                      cw_flag=concept_whitening,
...
class GCNNet(GNNBase):
    def __init__(self,
        ...
        if model_type == 'gcn':
            ...
            self.convs.append(GATConv(input_dim, self.gnn_latent_dim[0], heads=self.n_heads,
                              add_self_loops=self.add_self_loop))
        elif model_type == 'gin':
            self.convs.append(GINConv(nn.Linear(input_dim, self.gnn_latent_dim[0])))

        self.norm_layers.append(BatchNorm(self.n_heads*self.gnn_latent_dim[0])) ### Here you can change type of normalization
        ...
        for i in range(1, self.num_gnn_layers):
            if model_type == 'gcn':
                ...
                self.convs.append(GATConv(self.n_heads*self.gnn_latent_dim[i - 1], self.gnn_latent_dim[i], heads=self.n_heads,
                                  add_self_loops=self.add_self_loop))
            elif model_type == 'gin':
                self.convs.append(GINConv(nn.Linear(self.gnn_latent_dim[i - 1], self.gnn_latent_dim[i])))

            self.norm_layers.append(BatchNorm(self.n_heads*self.gnn_latent_dim[i])) ### Here you can change type of normalization
            
            
        # FC layers
        self.mlps = nn.ModuleList()
        if self.num_mlp_layers > 1:
            self.mlps.append(nn.Linear(self.emb_dim*self.n_heads, self.fc_latent_dim[0]))
            ...
            self.mlps.append(nn.Linear(self.emb_dim*self.n_heads, self.output_dim))
            
    def replace_norm_layers(self):
        for layer_idx in range(len(self.norm_layers)):
            self.norm_layers[layer_idx] = cw_layer(self.n_heads*self.gnn_latent_dim[layer_idx])
    ...
    def change_mode(self, mode):
        ...
        """
        for idx in range(len(self.norm_layers)):
            self.norm_layers[idx].mode = mode
    ...
    def update_rotation_matrix(self):
        ...
        """
        for idx in range(len(self.norm_layers)):
            self.norm_layers[idx].update_rotation_matrix()
    ...
    def get_emb(self, *args, **kwargs):
        ...
        for i in range(self.num_gnn_layers):
            ...
            x = self.convs[i](x, edge_index) # [*, 128] = [#atoms, latent_dim]
            #print(f'Output GCNConv: {x.shape}')
            
            if self.cw_flag:
                x = self.norm_layers[i](x, edge_index, batch)
            else:
                x = self.norm_layers[i](x)
            #print(f'Output CW: {x.shape}')
    ...
    def forward(self, *args, **kwargs):
        _, _, batch = self._argsparse(*args, **kwargs) # batch tells to which graph each atom belongs
        #print('INSIDE CONV NET')
        #print(f'batch: {batch.shape}') # [#atoms]
        
        # node embedding for GNN
        emb = self.get_emb(*args, **kwargs) # [*, 128] = [#atoms, latent_dim]
        #print(f'node embed: {emb.shape}')
        
        # pooling process
        x = self.readout_layer(emb, batch) # [32, 128] = [batch_size, latent_dim]
        ...
        logits = self.mlps[-1](x) # [32, 2] = [batch_size, classification logits]
        #print(f'After linear = {logits.shape}')
        
        if self.concept_acts:
            return emb, logits
